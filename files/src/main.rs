...
async fn main() -> anyhow::Result<()> {
    ...
    dotenv().ok();
    // let caldav_server_url = env::var("CALDAV_SERVER_URL").expect("CALDAV_SERVER_URL must be set");
    // let caldav_username = env::var("CALDAV_USERNAME").expect("CALDAV_USERNAME must be set");
    // let caldav_password = env::var("CALDAV_PASSWORD").expect("CALDAV_PASSWORD must be set");
